test_that("grouped definition varialbes work", {
  #'
  # We will demonstrate the use of definition variables with a latent growth curve
  # model. We assume that the six measurement occasions took place at group-specific
  # time points. There is two groups.
  ...
  times <- as.data.frame(
    matrix(seq(0,5,1),
      ...
      cbind(0,matrix(round(runif(n = 2*5, min = -.2,max = .2),2),
                     nrow = N,
                     ncol = 5,
                     byrow = TRUE)) # we add some jitter, but there is only two groups
  )
...
"
  ...
  testthat::expect_equal(length(dvarModel), 2)
